...
function [result para] = BlurEstimation_section_S2(Img,map)
t1 = clock();
Img = double(Img);
...
map = map-theta;
map(find(map>=0))=0;
map(find(map<0)) = -10000;

D_F_Ver2 = D_F_Ver(1:H-1,1:W-1)+ map;



D_F_Ver2(find(D_F_Ver2<0)) = 0;
S_D_Ver1 = sum(sum(D_F_Ver2));


D_F_Hor2 = D_F_Hor(1:H-1,1:W-1) +map;
D_F_Hor2(find(D_F_Hor2<0)) = 0;
S_D_Hor1 = sum(sum(D_F_Hor2));


V_Ver2 = V_Ver(1:H-1,1:W-1)+map;
V_Ver2(find(V_Ver2<0)) = 0;
S_V_Ver1 = sum(sum(V_Ver2));


V_Hor2 = V_Hor(1:H-1,1:W-1)+map;
V_Hor2(find(V_Hor2<0)) = 0;
S_V_Hor1 = sum(sum(V_Hor2));
